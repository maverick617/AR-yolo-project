        ...
        public void ApplyDetections(IReadOnlyList<DetectionResult> detections)
        {
            ...
            for (int i = 0; i < detections.Count; i++)
            {
                ...
                if (UsesDirectMultiTagCup(rule) && !tracked.HasMeasuredSize)
                {
                    if (axialMount)
                    {
                        SetStatus("SHOW SIDE APRILTAG 0/1/2 TO SIZE CUP");
                        LogDiagnostic(
                            "size:cup-bottom-view",
                            "Bottom AprilTag 3 can track a registered cup, but its bottom view cannot initialize YOLO cup height. Show side Tag 0, 1, or 2 for five frames.");
                        continue;
                    }
